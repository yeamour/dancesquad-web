function createTuanjieInstance(canvas, config, onProgress) {
  ...
  // Add fallback values for companyName, productName and productVersion to ensure that the TuanjieCache is working. 
  fallbackToDefaultConfigWithWarning(config, "companyName", "U3D");
  ...
  Module.QuitCleanup = function () {
    ...
    // After all deinitializer callbacks are called, notify user code that the Tuanjie game instance has now shut down.
  ...
  function errorHandler(message, filename, lineno) {
    // Tuanjie needs to rely on Emscripten deferred fullscreen requests, so these will make their way to error handler
    ...
    if (errorHandler.didShowErrorMessage)
      return;
    var message = "An error occurred running the content on this page. See your browser JavaScript console for more info. The error was:\n" + message;
...
Module.readBodyWithProgress = function() {
  ...
  function readBodyWithProgress(response, onProgress, enableStreaming) {
    ...
    if (!lengthComputable) {
      console.warn("[TuanjieCache] Response is served without Content-Length header. Please reconfigure server to include valid Content-Length for better download performance.");
    }
...
Module.TuanjieCache = function () {
  var TuanjieCacheDatabase = { name: "TuanjieCache", version: 4 };
  ...
  function log(message) {
    console.log("[TuanjieCache] " + message);
  }
  ...
  var instance = null;
  /**
   * Singleton accessor. Returns Tuanjie cache instance
   * @returns {TuanjieCache}
   */
  TuanjieCache.getInstance = function () {
    if (!instance) {
      instance = new TuanjieCache();
    }
  ...
  /**
   * Destroy Tuanjie cache instance. Returns a promise that waits for the
   * database connection to be closed.
   * @returns {Promise<void>}
   */
  TuanjieCache.destroyInstance = function () {
    if (!instance) {
  ...
  /**
   * Clear the Tuanjie cache. 
   * @returns {Promise<void>} A promise that resolves when the cache is cleared.
   */
  TuanjieCache.prototype.clearCache = function () {
    var self = this;
   ...
   */
  TuanjieCache.TuanjieCacheDatabase = TuanjieCacheDatabase;
  TuanjieCache.RequestMetaDataStore = RequestMetaDataStore;
  TuanjieCache.MaximumCacheSize = 1024 * 1024 * 1024; // 1 GB
   ...
   */
  TuanjieCache.prototype.loadRequest = function (request) {
    var self = this;
   ...
   */
  TuanjieCache.prototype.loadRequestMetaData = function (request) {
   ...
   * @returns {Promise<void>}
   */
  TuanjieCache.prototype.updateRequestMetaData = function (metaData) {
   ...
   * @returns {Promise<void>}
   */
  TuanjieCache.prototype.storeRequest = function (request, response) {
    var self = this;
   ...
   * @async
   */
   TuanjieCache.prototype.close = function () {
    return this.isConnected.then(function () {
      if (this.database) {
        this.database.close();
        this.database = null;
   ...
   */
  TuanjieCache.prototype.connect = function () {
    var self = this;
    ...
    var isConnected = new Promise(function (resolve, reject) {
      try {
        ...
        var openRequest = indexedDB.open(TuanjieCacheDatabase.name, TuanjieCacheDatabase.version);
      ...
      var cacheName = TuanjieCacheDatabase.name + "_" + Module.companyName + "_" + Module.productName;
   ...
   */
  TuanjieCache.prototype.upgradeDatabase = function (e) {
   ...
   */
   TuanjieCache.prototype.execute = function (store, operation, parameters) {
    return this.isConnected.then(function () {
      return new Promise(function (resolve, reject) {
        try {
  ...
  TuanjieCache.prototype.getMetaDataEntries = function () {
    var self = this;
    var cacheSize = 0;
    var metaDataEntries = [];
   ...
   * @returns {Promise<void>}
   */
  TuanjieCache.prototype.cleanUpCache = function () {
    var self = this;
    ...
    return this.getMetaDataEntries().then(function (result) {
      ...
      for (var i = 0; i < newMetaDataEntries.length; ++i) {
        if (cacheSize < TuanjieCache.MaximumCacheSize) {
          break;
        }
  ...
  var TuanjieCache = Module.TuanjieCache;
  ...
  function cachedFetch(resource, init) {
    var tuanjieCache = TuanjieCache.getInstance();
    ...
    function fetchAndStoreInCache(resource, init) {
      return fetch(resource, init).then(function (response) {
        ...
        if (response.status === 304) {
          ...
          tuanjieCache.updateRequestMetaData(cache.metaData).then(function () {
          ...
          return readBodyWithProgress(response, init.onProgress, init.enableStreamingDownload).then(function (response) {
            ...
            Promise.all([
              tuanjieCache.storeRequest(resource, clonedResponse),
              tuanjieCache.updateRequestMetaData(cache.metaData)
            ]).then(function () {
    ...
    return tuanjieCache.loadRequest(url).then(function (result) {
      ...
      if (cache.control == "immutable") {
        cache.revalidated = true;
        tuanjieCache.updateRequestMetaData(metaData).then(function () {
  ...
  function downloadBinary(urlId) {
      ...
      return request.then(function (response) {
        ...
        if (location.protocol == 'file:') {
          showBanner(error + '. Loading web pages via a file:// URL without a web server is not supported by this browser. Please use a local development web server to host content, or use the Build and Run option.', 'error');
        } else {
          console.error(error);
        }
      });
  ...
  function downloadFramework() {
      return new Promise(function (resolve, reject) {
        ...
        script.onload = function () {
          ...
          // 'tuanjieFramework' variable that should be considered internal.
          // If not, then we have received a malformed file.
          if (typeof tuanjieFramework === 'undefined' || !tuanjieFramework) {
            ...
            for(var i in compressions) {
              ...
              if (Module.frameworkUrl.endsWith('.' + compression[0])) {
                ...
                if (location.protocol == 'file:') {
                  showBanner(error + ' Loading pre-compressed (brotli or gzip) content via a file:// URL without a web server is not supported by this browser. Please use a local development web server to host compressed content, or use the Build and Run option.', 'error');
                  return;
                }
          ...
          // application quit.
          var fw = tuanjieFramework;
          tuanjieFramework = null;
  ...
  function loadBuild() {
    downloadFramework().then(function (tuanjieFramework) {
      tuanjieFramework(Module);
    });
    ...
    Module.preRun.push(function () {
      ...
      dataPromise.then(function (data) {
        ...
        var pos = 0;
        var prefix = "TuanjieWebData1.0\0";
...
function createUnityInstance(canvas, config, onProgress) {
  return createTuanjieInstance(canvas, config, onProgress);
}
